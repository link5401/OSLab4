...
void * best_fit_allocator(unsigned int size) {
	...
	struct mem_region * temp_region =free_regions;
	do {
		if (size <= temp_region->size) {
			if(current_region->size > temp_region->size) current_region = temp_region;
			found = 1;
		}
			temp_region = temp_region->next;
		
	} while (!found && temp_region != NULL);
